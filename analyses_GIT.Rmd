...
## geographical coordinates for spatial autocorr tests:
...
All Moran's I tests with our response variables to verify spatial auto-correlation resulted in non-significant which means non spatially auto-correlated. 
```{r}
...
(tab_gfs <- tab_model(g.fs, show.p = F, show.se = TRUE))
(tab_afs <- tab_model(a.fs, show.p = F, show.se = TRUE))
(tab_bfs <- tab_model(b.fs, show.p = F, show.se = TRUE))
(tab_brc1fs <- tab_model(rc1.fs, show.p = F, show.se = TRUE))
(tab_gabfs <- tab_model(gab.fs, show.p = F, show.se = TRUE))
(tab_brc2fs <- tab_model(rc2.fs, show.p = F, show.se = TRUE))


    # Nnfs
(tab_gnfs <- tab_model(g.nfs, show.p = F, show.se = TRUE))
(tab_anfs <- tab_model(a.nfs, show.p = F, show.se = TRUE))
(tab_bnfs <- tab_model(b.nfs, show.p = F, show.se = TRUE))
(tab_brc1nfs <- tab_model(rc1.nfs, show.p = F, show.se = TRUE))
(tab_gabnfs <- tab_model(gab.nfs, show.p = F, show.se = TRUE))
(tab_brc2nfs <- tab_model(rc2.nfs, show.p = F, show.se = TRUE))
```
...
(g.hab <- ggplot(data= new, aes(x= fc_5km, y = pred)) + 
    ...
    # scale_x_reverse() +
    annotate("text", y=20.5, x=40,colour= "#0072B2",
...
(gab.hab <- ggplot(new, aes(x=fc_5km, y=pred)) + 
  ...
  theme(axis.text=element_text(size=15),axis.title=element_text(size=14,face="bold"),legend.position = 'none') + 
  # scale_x_reverse() +
    annotate("text", y=400, x=40, colour= "#0072B2",
             label=expression(paste(~ R^2 ~ "= 0.44"))) +
    annotate("text", y=370, x=40, colour= "#D55E00",
...
(rc.hab <- ggplot(data= new, aes(x=fc_3km, y=pred)) + 
  ...
  theme(axis.text=element_text(size=14), axis.title=element_text(size=14,face="bold"),
    ...
    # scale_x_reverse() +
  annotate("text", y=0.9, x=40, colour= "#D33E00",
...
(rc.ab.hab <- ggplot(data= new, aes(x=fc_5km, y=pred)) + 
  ...
  theme(axis.text=element_text(size=14), axis.title=element_text(size=14,face="bold"),legend.position = 'none') +
    ylim(-1,1.1) + 
    # scale_x_reverse() +
  annotate("text", y=1, x=40, colour= "#0072B2",
           label=expression(paste(~ R^2 ~ "= 0.41"))) +
  annotate("text", y=0.85, x=40, colour= "#D55E00",
