...
1) conferência dos recálculos, alpha FS parou de responder
2) Olhar o código e resíduos, alguns já não parecem mais ok
3) marcar na planilha os bichos que mudaram
4) cálculo beta RC dentro do wrangling ou separado? onde jogo as funções (em script separado e usar source)
5) sugestão de como plotar quando tem nulo (linha tracejada mais clara e sem SE?)
6) Plot também vai pro Git? script separado?
...
(g.fs <- ggplot(new, aes(x=fc_5km, y=pred)) + geom_line() +
...
aic.tab <- rbind(aic.tab, 
                 ...
                 c(" ", "~ 1 (NULL)",
                   round(a.fs.aic$AICc[3],2),
                   round(a.fs.aic$dAICc[3],1), 
                   a.fs.aic$df[3],
                   round(a.fs.aic$weight[3],2),"-")
                 )
```
...
(a5k.fs <- ggplot(new, aes(x=fc_5km, y=pred)) + 
  geom_line() +
  geom_ribbon(aes(x=fc_5km , ymax=ic.up, ymin=ic.low), alpha=0.1) +
...
(b5k.fs <- ggplot(new, aes(x=fc_5km, y=pred)) + geom_line() +
  geom_ribbon(aes(x=fc_5km , ymax=ic.up, ymin=ic.low), alpha=0.1) +
...
aic.tab <- rbind(aic.tab, c("RCbeta (P/A-based) FS", 
                 ...
                 c(" ", "FC at 5km",
                   round(rc.fs.aic$AICc[3],2),
                   round(rc.fs.aic$dAICc[3],1),
                   rc.fs.aic$df[3],
                   round(rc.fs.aic$weight[3],2),
...
preds <- predict(rc3kq, type="response", newdata=new, se.fit=T)
...
(rc.rb.fs <- ggplot(new, aes(x=fc_3km, y=pred)) + geom_line() +
...
(gab.fs <- ggplot(new, aes(x=fc_5km, y=pred)) + 
...
(rc.ab.fs <- ggplot(new, aes(x=fc_5km, y=pred)) + geom_line() +
  geom_ribbon(aes(x=fc_5km , ymax=ic.up, ymin=ic.low), alpha=0.1) + 
...
(g3k.nfs <- ggplot(new, aes(x=fc_3km, y=pred)) + geom_line() +
  geom_ribbon(aes(x=fc_3km , ymax=ic.up, ymin=ic.low), alpha=0.1) +
...
## Plot Alpha richness ~ fc_5km
(a3k.nfs <- ggplot(new, aes(x=fc_3km, y=pred)) + 
  geom_line() +
  geom_ribbon(aes(x=fc_3km , ymax=ic.up, ymin=ic.low), alpha=0.1) +
...
(b3k.nfs <- ggplot(new, aes(x=fc_3km, y=pred)) + 
  geom_line() +
  geom_ribbon(aes(x=fc_3km , ymax=ic.up, ymin=ic.low), alpha=0.1) +
...
## Plot gamma richness ~ forest cover
(rc.rb.nfs <- ggplot(new, aes(x=fc_3km, y=pred)) + 
...
## Plot gamma richness ~ forest cover 5km
(gab.nfs <- ggplot(new, aes(x=fc_5km, y=pred)) + 
  geom_line() +
  geom_ribbon(aes(x=fc_5km , ymax=ic.up, ymin=ic.low), alpha=0.1) +
...
## Plot RCbeta abundance ~ forest cover at 5
(rc.ab.nfs <- ggplot(new, aes(x=fc_3km, y=pred)) + 
...
# aic.tab2 <- as.data.frame(aic.tab)
# library("writexl")
#write_xlsx(aic.tab2,here("updated_aic.tab2.xlsx"))
```
...
(g.hab <- ggplot(data= new, aes(x= fc_5km, y = pred)) + 
    ...
    annotate("text", y=19, x=40, colour= "#D55E00",
             label=expression(paste(~ R^2 ~ "= 0.74"))) + labs(tag="a"))
```
...
(a.hab <- ggplot(data= new, aes(x=fc_5km, y=pred)) + 
  geom_line(data= new, aes(x=fc_5km, y=pred), col= "#0072B2", alpha= .5, linetype= "dashed") +
  # geom_ribbon(data= new, aes(x=fc_5km , ymax=ic.up, ymin=ic.low), alpha=0.1, fill= "#0072B2") +
...
(rc.hab <- ggplot(data= new, aes(x=fc_3km, y=pred)) + 
  ...
  annotate("text", y=0.9, x=43, alpha= .5, colour= "#D33E00",
           label=expression(paste(~ R^2 ~ "= 0.17"))) +
